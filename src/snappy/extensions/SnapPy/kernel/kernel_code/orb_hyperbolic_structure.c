...
static SolutionType my_find_complete_hyperbolic_structure(
    Triangulation *manifold,
    ...
    Boolean       manual)
{
	...
	Real	step_size;	
        ...
        orb_initialize_shapes(manifold);
	...
	if (!manual)
	{
		...
		res = my_find_complete_hyperbolic_structure(manifold, FALSE, FALSE, approach_value);
	...
	while(approach_value >= 1)
	{
		if ( res == not_attempted )
			res = my_find_complete_hyperbolic_structure(manifold, TRUE, manual, approach_value);
...
static SolutionType prepare_for_manual( Triangulation *manifold )
{
	...
	if (need_to_perturb)
	{
		...
		if  (	edge->orb_is_singular )
		{
			...
			else	edge->orb_singular_order = edge->orb_old_singular_order;
		}
	...
	int i,j;
                ...
                for(i=0;i<6;i++)
		{
			...
			tet->orb_tet_shape->use_orientation_parameter[ultimate][i]
                ...
                else    tet->orb_tet_shape->Gram_matrix[i][i] = tet->cusp[i]->orb_cusp_shape->inner_product[ultimate];
...
static  FuncResult update_inner_products( Triangulation *manifold, Real *delta,int num_columns )
{
 ...
 for( cusp = manifold->cusp_list_begin.next;
      ...
      cusp = cusp->next)
 {
        if (max > MAX_STEP) delta[cusp->orb_cusp_shape->index] *= MAX_STEP / max;
...
static FuncResult update_Gram_matrices(Triangulation *manifold )
{
	...
	tet = tet->next)
 {
	...
	else	tet->orb_tet_shape->dihedral_angle[ultimate][i] = 0.0;
 }
     ...
     tet = tet->next)
 {
        ...
        if (tet->orb_tet_shape->use_orientation_parameter[penultimate][i])
        {
            ...
            else  tet->orb_tet_shape->use_orientation_parameter[ultimate][i] = FALSE;
        }
        else
        {
		...
		ABS(cos(tet->orb_tet_shape->dihedral_angle[ultimate][i])) < (1 / sqrt(2)))
            ...
            else  tet->orb_tet_shape->use_orientation_parameter[ultimate][i] = TRUE;
        }
	...
	if ( !used_t && !failed)  tet->orb_tet_shape->orientation_parameter[ultimate] =
...
extern void orb_compute_tilts( Triangulation *manifold)
{
	...
	for( f = 0; f<4;f++)
	{
		...
		else	tet->tilt[f] -= tet->orb_tet_shape->inverse_Gram_matrix[i][f];
		...
		if ( gl4R_determinant(tet->orb_tet_shape->Gram_matrix) * tet->orb_tet_shape->inverse_Gram_matrix[f][f] < 0 )
			factor = 0.0;
		else	factor = safe_sqrt( gl4R_determinant(tet->orb_tet_shape->Gram_matrix) * tet->orb_tet_shape->inverse_Gram_matrix[f][f]);	
